...
REGION: UI
TAG: CORE.AGENT_LEE_LEEWAY_CODING_SYSTEM.VSCODE_EXTENSION.SRC.CORE.BROWSER_ENGINE
...
export async function inspectVisualTarget(args: {
  ...
  const browser = await chromium.launch(browserLaunchOptions(browserExe, false, 0));
...
export async function runVisualUserFlow(args: {
  ...
  const browser = await chromium.launch(browserLaunchOptions(browserExe, headed, slowMoMs));
