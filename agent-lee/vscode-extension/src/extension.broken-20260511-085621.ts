...
const AGENT_LEE_UI_VERSION = "chat-ui-restored-voice-sync-2026-05-09";
...
function getHtml(webview: vscode.Webview, context: vscode.ExtensionContext) {
  const brandingIconUri = webview.asWebviewUri(
    vscode.Uri.joinPath(context.extensionUri, "media", "leeway-standards-activitybar.svg")
  );
  const standardsBtnUri = webview.asWebviewUri(
    vscode.Uri.joinPath(context.extensionUri, "media", "leeway-standards-button.svg")
  );
  return `
<!DOCTYPE html>
<html>
...
<div class="app-shell">
  <div class="topbar">
    ...
    <div class="top-actions">
      ...
      <button class="img-btn" id="topRightBtn" aria-label="Top Right Button" title="Top Right Button" style="margin-left:4px"><img src="${brandingIconUri}" alt="Top Right Button" style="height:24px" /></button>
    </div>
  </div>
      ...
      <div class="settings" id="settingsPanel" role="dialog" aria-modal="true" aria-label="Agent Lee settings">
        ...
        <div class="settings-grid">
          ...
          <div class="settings-card" style="margin-bottom:14px">
            ...
            <div class="settings-copy">Record or pick a reference audio clip (3–30s, clear speech, no background noise). Give it a name and transcript, then clone it. Clones are saved to your catalog.</div>
            <div class="settings-row" style="margin-top:10px">
    ...
    <div class="composer">
      ...
      <div class="footer">
        <div style="display:flex;align-items:center;gap:8px">
          <img src="${standardsBtnUri}" alt="LeeWay Standards" style="height:20px;border-radius:4px" />
        ...
        <div class="footer-right" style="display:flex;align-items:center;gap:8px">
          ...
          <button class="img-btn" id="bottomRightBtn" aria-label="Bottom Button" title="Bottom Button"><img src="${brandingIconUri}" alt="Bottom Button" style="height:20px" /></button>
        </div>
      </div>
    </div>
  </div>
...
function renderWorkers(state){
  ...
  root.innerHTML = getWorkerCatalog(state).map(function(entry){
    ...
    const responsibilities = (entry.responsibilities || []).slice(0,3).join(" · ");
    return '<div class="mcp-row">'
      + '<div><div class="mcp-name">'+escapeHtml(entry.name)+'</div><div class="mcp-desc">'+escapeHtml(description)+'</div><span class="agent-kind-pill">Leeway Worker</span><span class="agent-kind-pill">'+escapeHtml(entry.diagnosticLabel || "Worker")+'</span>'+(locked?'<span class="agent-lock-pill" title="'+escapeHtml(workerLockReason(entry))+'">Observed Only</span>':'')+'<div class="mcp-desc" style="margin-top:6px">Owner: '+escapeHtml(entry.owner || "agent-lee-prime")+' · Route: '+escapeHtml(entry.route || "unassigned")+'</div><div class="mcp-desc">'+escapeHtml(responsibilities)+'</div></div>'
...
// ─── Voice of Agent Lee tab functions ───
...
async function postRuntimeInfo(webview: vscode.Webview, prompt = "front-end task") {
  ...
  webview.postMessage({
    ...
    inventory: uiModelOptions.map((id) => ({
      ...
      ].filter(Boolean).join(" · ") || "system model"
    })),
    selection: {
      primaryModel: runtimeState.primaryModel,
      builderModel: runtimeState.builderModel,
...
async function handle(webview: vscode.Webview, msg: any, context?: vscode.ExtensionContext) {
  ...
  // ─── Voice of Agent Lee catalog handlers ───
