...
function Get-LatestInstalledDir {
  param([string[]]$ExtensionsRoots, [string]$PackageName)
...
$fileIdentityId = "LEEWAY_FILE::SCRIPT::INSTALLED_EXTENSION_CHECK"
$extensionsRoots = @(
  (Join-Path $env:USERPROFILE ".vscode\extensions"),
  (Join-Path $env:USERPROFILE ".vscode-insiders\extensions"),
  (Join-Path $env:USERPROFILE ".antigravity\extensions")
)
$installedCandidates = @(Get-LatestInstalledDir -ExtensionsRoots $extensionsRoots -PackageName $packageJson.name)
$installedCandidate = $installedCandidates | Select-Object -First 1
...
$duplicateInstallRoots = @($installedCandidates | Select-Object -ExpandProperty Root -Unique)
$duplicateInstallPaths = @($installedCandidates | ForEach-Object { $_.Directory.FullName })
...
if ($installedCandidates.Count -gt 1) { $staleReason += "MULTIPLE_INSTALLED_EXTENSION_ROOTS" }

$finalVerdict = if ($staleReason.Count -gt 0) { "FAIL" } else { "PASS" }
...
$result = [pscustomobject]@{
  ...
  installedCandidates = @($installedCandidates | ForEach-Object {
    [pscustomobject]@{
  ...
  duplicateInstallRoots = $duplicateInstallRoots
  duplicateInstallPaths = $duplicateInstallPaths
