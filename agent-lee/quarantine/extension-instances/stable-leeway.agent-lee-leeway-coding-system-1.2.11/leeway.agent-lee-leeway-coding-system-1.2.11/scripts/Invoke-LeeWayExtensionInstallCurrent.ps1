...
function Get-InstalledCandidates {
  param([string[]]$ExtensionsRoots)

  $candidates = foreach ($root in @($ExtensionsRoots | Where-Object { $_ -and (Test-Path -LiteralPath $_) })) {
...
$identityNodeId = "LEEWAY_APP::PACKAGE::INSTALLED_CLEANUP::STALE_INSTANCE_QUARANTINE"
$fileIdentityId = "LEEWAY_FILE::SCRIPT::INSTALL_CURRENT"
$legacyUpdateQuarantineId = "LEEWAY_QUARANTINE::PACKAGE::LEGACY_UPDATE_LANE"
...
$allExtensionRoots = @($extensionsRoot, $insidersExtensionsRoot, $antigravityExtensionsRoot)
$quarantineRoot = Join-Path $env:USERPROFILE ".leeway-vscode\agent-lee\quarantine\extension-instances"
$installedBefore = @(Get-InstalledCandidates -ExtensionsRoots $allExtensionRoots | ForEach-Object { $_.Name })
$foldersFound = @(
...
if (-not $codeCli) {
  ...
  $result = [pscustomobject]@{
    ...
    legacyUpdateQuarantineId = $legacyUpdateQuarantineId
...
foreach ($rootInfo in @(
  @{ Label = "stable"; Path = $extensionsRoot },
  @{ Label = "insiders"; Path = $insidersExtensionsRoot },
  @{ Label = "antigravity"; Path = $antigravityExtensionsRoot }
)) {
  foreach ($folder in (Get-ExtensionFolderInventory -RootPath $rootInfo.Path)) {
    if ($rootInfo.Label -eq "stable" -and $folder.Name -eq "leeway.agent-lee-leeway-coding-system-$($latestVsix.Version)") {
      $foldersKept.Add($folder.FullName)
      continue
    }
    $destination = Join-Path $quarantineRoot ("{0}-{1}" -f $rootInfo.Label, $folder.Name)
...
$installedAfter = @(Get-InstalledCandidates -ExtensionsRoots $allExtensionRoots)
